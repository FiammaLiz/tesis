function [params] = def_params2(params)
%% Default values
params.fs=params.fs;
params.ft=2*floor(0.5*(257/44100*params.fs))+1; % windowlen for any fs
...
params.window_th=50; %(in ms) 100ms va bien para P1
params.prop_th=3;
params.deadtime=(1E-3); % deadtime for on & off detection
params.min_duration=7.5E-3; % minimum duration of syllable to be considered
params.tolerance=3E-3; % tolerance for intrasyllabic breaking (less than param discarded)
params.d2promthreshm=0.2; % min prominence for finding minima in normalized d2
params.d2promthreshM=2; % min prominence for finding maxima in normalized d2
params.d2pkthresh=0.15; % also requires a minimum absolute value in amplitude
params.min_dist=2E-3; % minimum distance for instances to be considered
params.deadt=2.5E-3; % keep instance farther than params.deadt from onset and offsets
                     ...
                     % onset/offset)
params.eps=0.02; % epsilon for first deriv. to be considered "near zero"
params.wdw=round((1E-3)*params.fs); % window for eps to be considered
...
%% BF
if(strcmp(params.birdname(1:2),'bf') || strcmp(params.birdname(1:2),'BF'))
params.fs=params.fs;
params.ft=2*floor(0.5*(641/44100*params.fs))+1; % S-G windowlen for any fs
params.expf=10E-3; % for exponential filtering
params.smd=1E-3;  % for moving average smoothing of derivatives
params.o_thresh=0.1; % threshold for onset and offset detection. 
...
%% LF
if(strcmp(params.birdname(1:2),'lf') || strcmp(params.birdname(1:2),'LF'))
params.fs=params.fs;
params.ft=2*floor(0.5*(641/44100*params.fs))+1; % S-G windowlen for any fs
