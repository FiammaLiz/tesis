function [gtes,env_norm,d,d2,params] = find_gte(sound,params)
...
[~,offsets]=findpeaks(-diff(env_norm>params.o_thresh),'MinPeakDistance',round((params.deadtime)*params.fs));
% Corrects in case that detection yields different amount of onsets/offsets
if(length(onsets)<length(offsets))
 offsets_aux=zeros(1,length(onsets));
 for k=1:length(onsets)
 offsets_aux(k)=offsets(find((offsets-onsets(k))>round((params.deadtime)*params.fs),1,'first'));
 end
 offsets=offsets_aux;
end
if(length(offsets)<length(onsets))
 [~,onsetsb]=findpeaks(diff(env_norm>params.o_thresh));   
